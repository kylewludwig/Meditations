...
protocol TopicsPresentationLogic {
  func presentFetchedTopics(response: Topics.FetchTopics.Response)
  func presentFailureAlert(_ title: String, message: String?)
}

class TopicsPresenter: TopicsPresentationLogic {

  weak var viewController: TopicsDisplayLogic?

  // MARK: Presentation logic

  func presentFetchedTopics(response: Topics.FetchTopics.Response) {
    let viewModel = Topics.FetchTopics.ViewModel(topics: response.topics)
    viewController?.displayFetchedTopics(viewModel: viewModel)
  }
