...
protocol MeditationsPresentationLogic {
  func presentFetchedMeditations(response: Meditations.FetchMeditations.Response)
  func presentFailureAlert(_ title: String, message: String?)
}

class MeditationsPresenter: MeditationsPresentationLogic {

  weak var viewController: MeditationsDisplayLogic?
  
  // MARK: Presentation logic
  
  func presentFetchedMeditations(response: Meditations.FetchMeditations.Response) {
    let viewModel = Meditations.FetchMeditations.ViewModel(topic: response.topic)
    viewController?.displayFetchedMeditations(viewModel: viewModel)
  }

  func presentFailureAlert(_ title: String, message: String?) {
    viewController?.displayFailureAlert(title, message: message)
  }
}
